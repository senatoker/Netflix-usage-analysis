...
# Motivation
As someone who spends a considerable amount of time on Netflix, in this project, I want to better understand my viewing habits and preferences. This projects aims to analyze my Netflix data to uncover patterns such as:
...
# Data Sources
I have one main source of data and three other sources to deepining, cleaning and organizing the data.
...
I used my Netflix data and seeked for the consecutive days I watched the same series. Then I got their genres from IMDb data to understand which genres capture my attention the most.
...
# Data Visualization 
1) First with netflixanalysis.py, I obtained a list of genres and their counts in my viewving data: "Thriller": 73,
    "Drama": 71,
    "Mystery": 53,
    "Comedy": 49,
    "Romance": 35,
...
2) In order to test my hypothesis:

H0: There is no correlaton between IMDb ratings and my viewing habits.

I wroted the python code imdbratings.py and created a scatter chart. I also calculated the Spearman correlation coefficient, and since it was too small (0.23), it was seen that there is no any significant correlation between IMDb ratings and my watching habits and we can accept the null hypothesis. 
...
3) Other than that, I seeked for consecutive days I watched the same show with bingewatching.py, and also visualized this data. I looked for 5 or more consecutive days I watched the same thing. This data showed that, even though I mostly watch genres thriller and drama, I liked to watch comedy and drama consecutively and they capture my attention the most. 
...
4) Lastly, to analyze the relationship between the days of the week and my viewing habits, I plotted a bar graph showing the number of views for each day of the week. The x-axis represents the days (Monday to Sunday), while the y-axis represents the number of views.
...
While the project utilized data from Netflix, IMDb, and other APIs, the reliance on external APIs for genres and IMDb ratings led to issues in retrieving accurate or complete metadata. This affected the richness of the analysis, especially for genres and ratings. 

##  Genre Classification
Due to the complexity of multi-genre titles, the manual splitting of genres could result in inaccuracies or oversimplifications, particularly when a title has more than two genres. This might affect the granularity of the genre analysis.

## Possible Bias in Binge-Watching Analysis
The criteria for binge-watching (watching the same show for 5 or more consecutive days) may not necessarily reflect the most accurate portrayal of binge-watching behaviors, as watching different shows on consecutive days may also represent binge-like behavior.

## Future Work
Future analyses could benefit from using machine learning models (such as clustering or recommendation algorithms) to uncover deeper patterns in the viewing habits, genres, and ratings correlations.
